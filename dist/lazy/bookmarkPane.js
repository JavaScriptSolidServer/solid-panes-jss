...
export const bookmarkPane = {
  ...
  label: function (subject, context) {
    ...
    if (kb.holds(subject, rdfType, ns.ldp('Container')) || kb.holds(subject, rdfType, ns.ldp('BasicContainer'))) {
      const contents = kb.each(subject, ns.ldp('contains'));
      const hasBookmarks = contents.some(item => kb.holds(item, rdfType, bookmarkType));
      if (hasBookmarks) {
        return 'Bookmarks';
      }
    }
  ...
  render: async function (subject, context, options) {
    try {
      const mod = await import(/* webpackChunkName: "bookmark-pane" */
      './bookmarkRender.js');
  ...
  mintNew: async function (context, options) {
    const {
      mintNew
    } = await import(/* webpackChunkName: "bookmark-pane" */
    './bookmarkRender.js');
...
//# sourceMappingURL=bookmarkPane.js.map